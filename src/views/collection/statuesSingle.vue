      ...
      >
        ...
        <div class="w-100 gap-24 exh-items">
          ...
          <div class="customGrid-r-w w-100 d-f fd-c gap-8">
            ...
            <p v-if="allInfos?.additional?.purchasedDate" class="commonP bold colorType">{{ filPost(allInfos?.additional?.purchasedDate) + " " + $t("year2") }}</p>
          </div>
          <div class="customGrid-r-w w-100 d-f fd-c gap-8">
...
export default {
  ...
  methods: {
    async getSingleExhibits() {
      await this.$api.get('/collections/statue/site/' + this.$route.query.exId)
          .then(resp => {
